...
export function PoolsDashboard({ userAddress }: PoolsDashboardProps) {
  ...
  if (poolsFetching)
    return <div className="text-center">Завантаження даних про пули...</div>;

  return (
    <div className="space-y-8">
      <Card className="bg-gradient-to-r from-blue-900 to-indigo-900 text-white shadow-xl">
        <CardHeader>
          <CardTitle>Ваша Ліквідність</CardTitle>
          <CardDescription className="text-gray-300">
            Сумарна вартість ваших часток у всіх пулах.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="text-4xl font-extrabold mb-2">
            {sharesFetching ? "Завантаження..." : `$${totalUserValue.toFixed(2)}`}
          </div>
          {!userAddress && (
            <p className="text-yellow-400 mt-4">
              Підключіть гаманець для перегляду ваших часток.
            </p>
          )}
        </CardContent>
      </Card>

      <h2 className="text-2xl font-semibold mt-10">Доступні пули ліквідності</h2>
      <Separator />
        ...
        {pools.map((pool: LiquidityPool) => {
          ...
          return (
            <Card key={pool.id} className="hover:shadow-lg transition-shadow">
              <CardHeader>
                <CardTitle>{pool.assetSymbol} пул</CardTitle>
