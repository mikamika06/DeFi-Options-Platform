...
export function IVSmileChart({ data, underlying, expiry }: IVSmileChartProps) {
  ...
  React.useEffect(() => {
    ...
    console.log(`Рендеринг IV Smile для ${underlying} @ ${expiry}:`, data);
    ...
    <div className="space-y-4">
      ...
      >
        <p className="text-muted-foreground">
          {data.length > 0
            ? "Графік IV Smile (TradingView Charts Placeholder)"
            : "Виберіть актив та експірацію для візуалізації"}
        </p>
      </div>

      {data.length > 0 && (
        <div className="max-h-40 overflow-y-auto border rounded-lg p-2">
          <p className="font-medium text-sm mb-1">Дані IV (Strike {"->"} IV)</p>
